...
const skillGroups = [
  ...
  { name: "Security & Tooling", icon: ShieldCheck, skills: ["SonarQube", "Trivy", "Falco"] },
];

const socialIcons: Record<string, string> = { GitHub: "GitHub", LinkedIn: "LinkedIn", Medium: "Medium" };
    ...
    <main className="shell home">
        ...
        <div className="hero-grid">
          ...
          <div>
            ...
            <div className="socials" aria-label="Social links">
              {site.social.filter(([n]) => n !== "Email").map(([n, u]) => (
                <a key={n} href={u} target={u.startsWith("http") ? "_blank" : undefined} rel="noreferrer" aria-label={n} title={n}>
                  <TechIcon name={socialIcons[n] ?? n} size={18} />
                </a>
              ))}
            </div>
          </div>
      ...
      <Section command="cat about.md" title="About Me" id="about">
        <div className="about-grid"><div>
          <p>Senior DevOps Engineer with 7+ years of experience building cloud-native platforms and automating software delivery.</p>
          <p>My work focuses on AWS, Kubernetes, Terraform, GitOps, CI/CD, observability, and DevSecOps. I document what I build through practical tutorials and projects.</p>
          <p>I prefer infrastructure that is reproducible, deployments that are boring, and systems that are easier to operate than they were yesterday.</p>
        </div><div className="stats"><div><strong>7+</strong><span>years experience</span></div><div><strong>AWS</strong><span>cloud platform</span></div><div><strong>K8s</strong><span>platform focus</span></div><div><strong>IaC</strong><span>Terraform driven</span></div></div></div>
      </Section>

      <Section command="kubectl get skills" title="Skills & Technologies" id="skills">
        <p className="section-lead">The stack I use to build, automate, secure, and operate cloud-native systems.</p>
        <div className="skill-categories">
          {skillGroups.map((group) => { const GroupIcon = group.icon; return (
            <div className="skill-category" key={group.name}>
              <div className="skill-category-head"><span className="skill-category-icon"><GroupIcon size={19} /></span><strong>{group.name}</strong></div>
              <div className="skill-items">{group.skills.map((skill) => <span className="skill-item" key={skill}><TechIcon name={skill} size={17} />{skill}</span>)}</div>
            </div>
          ); })}
        </div>
      </Section>

      <Section command="ls ~/projects --sort=impact" title="Projects" id="projects">
        <p className="section-lead">Hands-on infrastructure and platform projects built around Kubernetes, AWS, GitOps, observability, and DevSecOps.</p>
        <div className="project-grid">{projects.map((p) => <ProjectCard key={p.route} project={p} />)}</div>
      </Section>

      <Section command="git log --work" title="Experience" id="experience"><div className="timeline">{site.experience.map((x) => <div className="timeline-item" key={x.company}><div className="timeline-marker" /><div><div className="timeline-head"><h3>{x.role} · <a href={x.url} target="_blank" rel="noreferrer">{x.company}</a></h3><span>{x.date}</span></div><ul>{x.bullets.map((b) => <li key={b}>{b}</li>)}</ul></div></div>)}</div></Section>
